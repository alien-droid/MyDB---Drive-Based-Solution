...
export const uploadFile = async ({
  ...
  try {
    const inputFile = InputFile.fromBuffer(file, file.name); // appwrite file object
    ...
    const newFile = await databases
      ...
      .catch(async (error: unknown) => {
        await storage.deleteFile(appwriteConfig.bucketId, bucketFile.$id); // delete file from storage if db fails
        handleError(error, "Failed to create file in db");
      });
...
const createQueries = (user: Models.Document, types: string[], searchText: string, sort: string, limit?: number) => {
  ...
  queries.push(sortOrder === "asc" ? Query.orderAsc(sortBy) : Query.orderDesc(sortBy));
  return queries;
};
